...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
		...
		for (int i = 1; i < (rows + 1); i++) {
			for (int j = 1; j < (cols + 1); j++) {
				if (lineString != null) {
					if (j - 1 < lineString.length() && (char) (lineString.charAt(j - 1)) == 'x') {
						board[i][j] = 1;
					} else {
						board[i][j] = 0;
					}
	...
	public static int cellValue(int[][] board, int i, int j) {
		//// Replace the following statement with your code.
		int counterLivingNeighbors = count(board, i, j);
		if (board[i][j] == 1) {
			if (counterLivingNeighbors < 2) {
				return 0;
			} else if (counterLivingNeighbors > 3) {
				return 0;
			} else {
				return 1;
			}
		...
		for (int row = i - 1; row < i + 2; row++) {
			for (int col = j - 1; col < j + 2; col++) {
				...
				if (row >= 0 && row < board.length && col >= 0 && col < board[0].length) {
					// Check if the neighboring cell is a living cell
					if (board[row][col] == 1) {
